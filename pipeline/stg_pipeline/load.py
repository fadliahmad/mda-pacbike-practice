...
class Load_STG(luigi.Task):
    
    def requires(self):
        return [Extract_DB(), Extract_API(data_name = "currency")]
    ...
    def run(self):
        ...
        db_input, api_input = self.input()[0], self.input()[1]

    
        # Read Data to be load
        try:
            # Read CSV File
            df_currency = pd.read_csv(api_input[0].path)
            df_person = pd.read_csv(db_input[0].path)
            df_employee = pd.read_csv(db_input[1].path)
            df_product = pd.read_csv(db_input[2].path)
            df_shipmethod = pd.read_csv(db_input[3].path)
            df_currencyrate = pd.read_csv(db_input[4].path)
            df_salesreason = pd.read_csv(db_input[5].path)
            df_salesterritory = pd.read_csv(db_input[6].path)
            df_specialoffer = pd.read_csv(db_input[7].path)
            df_salesperson = pd.read_csv(db_input[8].path)
            df_store = pd.read_csv(db_input[9].path)
            df_customer = pd.read_csv(db_input[10].path)
            df_specialofferproduct = pd.read_csv(db_input[11].path)
            df_salespersonquotahistory = pd.read_csv(db_input[12].path)
            df_salesterritoryhistory = pd.read_csv(db_input[13].path)
            df_shoppingcartitem = pd.read_csv(db_input[14].path)
            df_salesorderheader = pd.read_csv(db_input[15].path)
            df_salesorderdetail = pd.read_csv(db_input[16].path)
            df_salesorderheadersalesreason = pd.read_csv(db_input[17].path)
        ...
        #----------------------------------------------------------------------------------------------------------------------------------------
        # Establish connections to STG
        try:
            _, stg_engine, _ = db_connection()
            logging.info(f"Connect to STG - SUCCESS")
            
        except Exception:
            logging.info(f"Connect to STG - FAILED")
            raise Exception("Failed to connect to Staging")
        
        #----------------------------------------------------------------------------------------------------------------------------------------
        # Record start time for loading tables
        start_time = time.time()  
        full_log = []
        logging.info("==================================STARTING LOAD DATA TO STG=======================================")
        ...
        for i, df in enumerate(list_df):
            ...
            pd.DataFrame(full_log).to_csv(f"{DIR_TEMP_LOG}/stg_load_done.csv", index=False)
            # Record end time for loading tables
            end_time = time.time()  
            execution_time = end_time - start_time  # Calculate execution time
            
        logging.info("==================================ENDING LOAD DATA TO STG=======================================")
        
    #----------------------------------------------------------------------------------------------------------------------------------------
    def output(self):
        return luigi.LocalTarget(f"{DIR_TEMP_LOG}/stg_load_done.csv")
